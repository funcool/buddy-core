...
(def engine (kdf/engine {:alg :hkdf+sha256
                         :key "mysecret"
                         :salt "mysalt"}))

(-> (kdf/get-bytes engine 8
    (codecs/bytes->hex)))
;; => "0faba553152fce4f"
----

If you do not happy with the current variants, you can use any other hash algorithm
already supported in buddy under `buddy.core.hash` namespace or directly passing
a digest engine instance:

[source, clojure]
----
(def engine (kdf/engine {:alg :hkdf
                         :digest :blake2-512
                         :key "mysecret"
                         :salt "mysalt"}))
----
